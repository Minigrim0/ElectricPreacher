 ...
 *
 * @param props window properties pointer
 */
Screen::Screen(const WindowProps &props)
    : m_width(props.width), m_height(props.height), m_tile_size(32), m_start_time(0),
      m_time_elapsed(0), m_time_since_last_fps_update(0), m_fps(0), m_showing_fps(false),
      m_window_caption(props.title), m_keyConf(std::map<SDL_Keycode, bool>()),
      m_fps_texture(nullptr), m_fps_surface(nullptr), m_window(nullptr), m_Renderer(nullptr),
      m_font_color({255, 255, 255, 255}), m_background_color({0, 0, 0, 0}),
      m_fonts(std::map<std::string, TTF_Font *>()), m_mouse_pos({0, 0, 0, 0}),
      m_fps_pos({0, 0, 0, 0}) {}

/**
 * @brief Destroy the Screen:: Screen object
 *
 * Fonts added to the screen will be freed here.
 */
Screen::~Screen() {
    ME_CORE_INFO("Deleting screen");
    m_fonts.clear();
...
Screen *Screen::create(const WindowProps &props) { return new Screen(props); }
...
// Setters
void Screen::setWidth(int width) { m_width = width; }

void Screen::setHeight(int height) { m_height = height; }

void Screen::setBackgroundColor(SDL_Color color) { m_background_color = color; }
...
void Screen::setDefaultFont(std::string font_id) { m_fonts["default"] = m_fonts[font_id]; }
...
int Screen::buildWindow() {
    ...
    m_window = SDL_CreateWindow(m_window_caption.c_str(), SDL_WINDOWPOS_UNDEFINED,
...
SDL_Surface *Screen::renderTextBlend(std::string text) {
    return TTF_RenderText_Blended(m_fonts["default"], text.c_str(), m_font_color);
}

SDL_Surface *Screen::renderTextBlend(std::string text, TTF_Font *font, SDL_Color color) {
    if (font == nullptr) {
...
SDL_Surface *Screen::renderTextSolid(std::string text) {
    return TTF_RenderText_Solid(m_fonts["default"], text.c_str(), m_font_color);
}

SDL_Surface *Screen::renderTextSolid(std::string text, TTF_Font *font, SDL_Color color) {
