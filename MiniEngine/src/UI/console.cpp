...
Console::Console(int x, int y, int w, int h)
    : m_rect({x, y, w, h}), m_history(std::vector<Widgets::Button *>()), m_font(nullptr),
      m_background_texture(nullptr), m_input(new Widgets::TextInput),
      m_send_button(new Widgets::Button), m_nb_visible_lines(5), m_line_height(0), m_alpha(125) {
    init_send_button();
}

Console::Console(TTF_Font *font)
    : m_rect({0, 0, 0, 0}), m_history(std::vector<Widgets::Button *>()),
      m_input(new Widgets::TextInput), m_send_button(new Widgets::Button), m_font(font),
      m_background_texture(nullptr), m_nb_visible_lines(5), m_line_height(0), m_alpha(125) {
...
void Console::init(Screen *screen) {
    ...
    m_nb_visible_lines =
        static_cast<int>(static_cast<float>(m_rect.h) / static_cast<float>(m_line_height)) - 1;
...
void Console::OnRender(Screen *screen) {
    ...
    int start = m_history.size() > static_cast<long unsigned int>(m_nb_visible_lines)
                    ? static_cast<int>(m_history.size()) - m_nb_visible_lines
                    : 0;
    for (int x = start; x < static_cast<int>(m_history.size()); x++) {
        m_history[static_cast<long unsigned int>(x)]->OnRender(
            screen); // draw_offset_y(screen, x*30);
    }
...
void Console::OnTextSubmit(std::string text) {
    ...
    int max = static_cast<int>(m_history.size()) < m_nb_visible_lines
    ...
    tmp_button->set_rect(m_rect.x, m_rect.y + m_line_height * (m_nb_visible_lines - 1), m_rect.w,
                         m_line_height);
