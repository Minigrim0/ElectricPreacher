...
void save_texture(Screen *screen, SDL_Texture *texture, const char *filename) {
    ...
    SDL_RenderReadPixels(screen->get_renderer(), NULL, surface->format->format, surface->pixels,
                         surface->pitch);
...
void save_texture(SDL_Surface *surf, const char *filename) { SDL_SaveBMP(surf, filename); }
} // namespace Utils
} // namespace MiniEngine
