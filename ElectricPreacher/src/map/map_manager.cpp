...
MapManager::MapManager()
    : m_default_missing(nullptr), m_position({0, 0}), m_origin({0, 0}),
      ...
      m_tilesets(std::map<std::string, MiniEngine::Graphics::TileSet *>()), m_tilewidth(32),
      m_tileheight(32), chunk_width(16), chunk_height(16) {}

MapManager::MapManager(int x, int y) : MapManager() { set_position(x, y); }

MapManager::MapManager(SDL_Rect position) : MapManager() { set_position(position); }

MapManager::~MapManager() {}

// Getters
SDL_Rect MapManager::get_position() const { return m_position; }

// Setters
void MapManager::set_position(SDL_Rect pos) { m_position = pos; }

void MapManager::set_position(int x, int y) { m_position = {x, y}; }
...
bool MapManager::OnEvent(SDL_Event *event) { return false; }
