 ...
 */
void Chunk::load(nlohmann::json chunk,
                 std::map<std::string, MiniEngine::Graphics::TileSet *> *tilesets,
    ...
    m_elements =
        static_cast<MapElement **>(malloc(m_chunk_size.x * m_chunk_size.y * sizeof(MapElement)));
    ...
    for (int y = 0; y < m_chunk_size.y; y++) {
        for (int x = 0; x < m_chunk_size.x; x++) {
            GroundElement *newElem =
                new GroundElement({x * tileset->get_tile_width(), y * tileset->get_tile_height()});
            newElem->set_texture(
